...
def load_pickle():
    with open(get_file_path("forecast2/loop_state2.pickle"), "rb") as f:
        state = pickle.load(f)
        current_tuner_index, current_scaler_index = state
        print(f"Resuming from: (tuner: {current_tuner_index} and scaler: {current_scaler_index})")
    return current_tuner_index, current_scaler_index

# Create Pickle save function
def save_pickle():
    with open(get_file_path("forecast2/loop_state2.pickle"), "wb") as f:
        pickle.dump((current_tuner_index, current_scaler_index), f)
        print(f"Saved state: (tuner: {current_tuner_index} and scaler: {current_scaler_index})")

# Load model before starting the loop
try:
    current_tuner_index, current_scaler_index = load_pickle()
except:
    save_pickle()

# Iterate over each tuner and scaler
for tuner in tuners[current_tuner_index:]:
    for name, scaler in list(scalers.items())[current_scaler_index:]:
        ...
        # Print current tuner and scaler
        print(f"Current tuner: {tuner}")
        print(f"Current scaler: {name}")

        # Specify model directory and project name based on tuner and scaler
        model_dir = get_file_path("forecast2/models")
        project_name = f"{tuner}_{name}_{currentTime}"
        checkpoint_name = f"{tuner}_{name}"

        # Find the previous run
        latest_model_path = os.listdir(get_file_path("forecast2/models"))
        ...
        if latest_model is not None:
            tensorboard_dir = os.path.join(get_file_path("forecast2/tensorboard"), latest_model)
            old_tb_dir = os.path.join(get_file_path("forecast2/old tb"), latest_model)
            # Redefine TensorBoard callback
            tensorboard_callback = tf.keras.callbacks.TensorBoard(
                log_dir=os.path.join(get_file_path("forecast2/tensorboard"), latest_model), histogram_freq=50, 
                write_graph=True, write_images=True, update_freq='batch', 
                profile_batch=1, write_steps_per_second=False
            )
        else:
            # Specify TensorBoard directory based on tuner and scaler
            tensorboard_dir = os.path.join(get_file_path("forecast2/tensorboard"), project_name)
            old_tb_dir = os.path.join(get_file_path("forecast2/old tb"), project_name)
        ...
        checkpointing = tf.keras.callbacks.BackupAndRestore(
            backup_dir=get_file_path(get_file_path("forecast2/checkpoints", checkpoint_name)),
            save_freq='epoch'
        )
